...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    # do not take batch into consider
    loss = 0
    for i in range(len(Z)):
        y_pred = Z[i]
        y_true = y[i]
        loss += -y_pred[y_true] + np.log(np.sum(np.exp(y_pred)))
    # loss = Z[np.arange(y.shape[0]), y] + np.log(np.sum(np.exp(Z), axis=1))
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, len(X), batch):
        ...
        out = np.matmul(X_batch, theta)
        loss = softmax_loss(out, y_batch)
        # breakpoint()

        out_grad = softmax(out) - np.eye(out.shape[1])[y_batch]
        theta_grad = (X_batch.T @ out_grad )/ (end - start)
        theta -= lr * theta_grad
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, len(X), batch):
        ...
        z1 = relu(X_batch @ W1)
        g2 = softmax(z1 @ W2) - np.eye(W2.shape[1])[y_batch]
        g1 = g2 @ W2.T * (z1 > 0)
